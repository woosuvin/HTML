...
console.log('result = ', result); // -> NaN(Not a Number) (1 + undefined)
// 전달하지 않은 parameter의 값은 undefined가 됨.

// 자바스크립트의 모든 함수는 arguments 이름의 프로퍼티property를 가지고 있음.
...
/*
 ...
 1. 함수는 프로퍼티(property - 자바의 field)를 가질 수 있음. (예) arguments
 2. 함수는 변수에 저장할 수 있음.
...
const plus = add; // 함수를 변수에 저장. =add();는 add함수의 리턴값을 plus에 저장한 것.
...
console.log('result= ', result);
console.log('calculate= ', calculate(1, 5, add));


result = calculate(5, 2, function(x, y) {
    return x * y;
})
console.log('result= ', result);
...
const increaseTen = increase(10); // x + 10 리턴
console.log(increaseTen); //ƒ addN(x) {return x + n;}
console.log(increaseTen(1)); // 11

const increaseOne = increase(1); // x + 1 리턴
console.log(increaseOne);
console.log(increaseOne(5)); // 6
...
console.log(fnAdd(3, 4)); // 7

const fnSubtract = (x, y) => x - y;
console.log(fnSubtract(3, 4)); // -1

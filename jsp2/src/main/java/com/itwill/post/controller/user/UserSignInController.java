...
public class UserSignInController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    String username = request.getParameter("username"); // getParameter(jsp에서 name에 쓴 값);
	    ...
	    if (user == null) { // username or password가 일치하지 않는 경우 -> 로그인 실패, null
	        // 로그인 페이지로 이동(redirect)
	        response.sendRedirect("/post/user/signin");
	        // context-root(/post) / path(/user/signin) ?query-string

...
document.addEventListener('DOMContentLoaded', function () {
    ...
    btnDelete.addEventListener('click', (e) => {
        ...
        //    preventDefault는 버튼의 기본 동작이 기능하지 않도록 함.
        
        const id = inputId.value;
        const result = confirm(`NO. ${id} 정말 삭제할까요?`); // 확인 true 취소 false 리턴.       
    ...
    btnUpdate.addEventListener('click', (e) => {
        ...
        if (title === '' || content === '') { // 제목 또는 내용이 비어 있으면
        ...
        if (result) {
            form.action = 'update'; // 폼 제출(요청) 주소, controller
            form.method = 'post'; // 요청 방식
            form.submit(); // 폼 제출(요청 보내기)
        }
    });
